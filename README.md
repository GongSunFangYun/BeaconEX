...
## 待实现/修复
- [X] 优化 world_analyzer 模块的输出
- [X] 优化 world_backup 模块的参数与输出
- [X] 优化 query_server 模块的输出与数据处理
- [X] 修复 rcon_executor 模块的EOF问题
- [X] 修复 dll_injector 模块配置文件无法保存的问题
- [X] 移除 dll_injector 模块计时相关的功能
- [X] 新增 dll_injector 模块对目标进程的存储功能
- [X] 简化 dll_injector 模块的参数与细化注入过程输出
- [X] 优化 playerdata_counter 模块的API和参数
- [X] 优化 make_icon 模块的输出
...
| `beaconex-linux-armv7-v301` | Linux | ARMv7 | 主程序 |
| `beaconex-linux-riscv64-v301` | Linux | RISC-V | 主程序 |
| `beaconex-darwin-x86_64-v301` | macOS | Intel | 主程序 |
...
# 指定进程名注入
bex d -d ./mod.dll -p Minecraft.Windows.exe
...
bex b -b ./server -t worlds -v 1h -l -x 10
