...
**Linux (ARM架构)**
```bash
GOOS=linux GOARCH=arm64 go build -o bex bex.go
```
...
do
    ...
    output_name="bex-${GOOS}-${GOARCH}"
    if [ $GOOS = "windows" ]; then
        output_name+=".exe"
    fi
    GOOS=$GOOS GOARCH=$GOARCH go build -o $output_name bex.go
    echo "已编译: $output_name"
done
```
