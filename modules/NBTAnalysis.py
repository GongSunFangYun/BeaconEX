...
class NBTDataParser:
    def __init__(self, file_path: str):
        ...
        self.player_data = None # 玩家数据
        self.player_name = self._GetPlayerName() # 玩家名称赋值函数
        self.api_cache = {}  # API缓存

    def _GetUsernameByUUID(self, uuid: str) -> str: # 通过请求Mojang API获取玩家名称
        # 检查缓存
        if uuid in self.api_cache:
            return self.api_cache[uuid]
        ...
        try:
            ...
            # 设置超时和请求头
            headers = {
                'User-Agent': 'BeaconEX NBT Parser/1.0'
            }
            ...
            if response.status_code == 200:
                ...
                if username:
                    # 缓存成功的结果
                    self.api_cache[uuid] = username
                    return username

        except Exception as e:
            log_warn(f"处理API响应时发生错误: {e}，使用UUID作为玩家名称")

        # 请求失败或没有名称，返回UUID
        self.api_cache[uuid] = uuid
        return uuid
    ...
    def ParseNBTData(self) -> bool:
        try:
            ...
            if 'bukkit' in root and 'lastKnownName' in root['bukkit']:
                ...
                # 如果NBT文件中没有名称，尝试通过UUID获取
                uuid_from_file = self._ExtractUUIDFromFile()
                if uuid_from_file:
                    self.player_name = self._GetUsernameByUUID(uuid_from_file)
    ...
    def _ExtractUUIDFromFile(self) -> str:
        # 首先尝试从文件名提取UUID
        filename = os.path.basename(self.file_path).replace('.dat', '')

        # 检查文件名是否是有效的UUID格式（带连字符或不带都可以）
        if self._IsValidUUID(filename):
            return filename

        # 如果文件名不是UUID，尝试从NBT数据中获取
        if self.player_data:
            # 尝试直接获取UUID字段
            if 'UUID' in self.player_data:
                uuid_value = self.player_data['UUID']
    ...
    @staticmethod
    def _IsValidUUID(uuid_str: str) -> bool:
        import re
        # 匹配带连字符的UUID格式：xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx
        uuid_pattern = re.compile(r'^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$', re.IGNORECASE)
        # 匹配不带连字符的UUID格式：xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
        uuid_pattern_no_dash = re.compile(r'^[0-9a-f]{32}$', re.IGNORECASE)

        return bool(uuid_pattern.match(uuid_str) or uuid_pattern_no_dash.match(uuid_str))
    ...
    def _GetPlayerBasicDatas(self):
        # 获取UUID用于显示
        display_uuid = "未知"
        if self.player_data.get('UUID'):
            uuid_value = self.player_data['UUID']
            if hasattr(uuid_value, 'hex'):
                display_uuid = str(uuid_value)
            else:
                display_uuid = str(uuid_value)

        data = {
            "玩家名称": f"{TextFormat.Colorize(self.player_name, TextFormat.GREEN)}",
            "UUID": f"{TextFormat.Colorize(display_uuid, TextFormat.YELLOW)}",
            "生命值": f"{TextFormat.Colorize(f'{float(self.player_data.get("Health", 20.0)):.1f}', TextFormat.YELLOW)}/20.0",
            "饥饿度": f"{TextFormat.Colorize(int(self.player_data.get('foodLevel', 20)), TextFormat.YELLOW)}/20",
            "饱和度": f"{TextFormat.Colorize(f'{float(self.player_data.get("foodSaturationLevel", 5.0)):.1f}', TextFormat.YELLOW)}",
        ...
        ability_data = {
            "行走速度": f"{TextFormat.Colorize(f'{float(abilities.get("walkSpeed", 0.1)) * 100:.0f}%', TextFormat.BLUE)}",
            "飞行速度": f"{TextFormat.Colorize(f'{float(abilities.get("flySpeed", 0.05)) * 100:.0f}%', TextFormat.BLUE)}",
            "允许飞行": TextFormat.Colorize("是", TextFormat.GREEN) if abilities.get('mayfly',
                                                                                     False) else TextFormat.Colorize(
                "否", TextFormat.RED),
            "正在飞行": TextFormat.Colorize("是", TextFormat.GREEN) if abilities.get('flying',
                                                                                     False) else TextFormat.Colorize(
                "否", TextFormat.RED),
            "无敌": TextFormat.Colorize("是", TextFormat.GREEN) if abilities.get('invulnerable',
                                                                                 False) else TextFormat.Colorize("否",
                                                                                                                 TextFormat.RED),
            "可以建造": TextFormat.Colorize("是", TextFormat.GREEN) if abilities.get('mayBuild',
                                                                                     True) else TextFormat.Colorize(
                "否", TextFormat.RED),
            "即时建造": TextFormat.Colorize("是", TextFormat.GREEN) if abilities.get('instabuild',
                                                                                     False) else TextFormat.Colorize(
                "否", TextFormat.RED)
        }
    ...
    def _GetPlayedTime(self) -> str:
        ...
        if first_played == 0 or last_played == 0:
            return TextFormat.Colorize("未知", TextFormat.RED)  # 未知用红色

        play_time_hours = (last_played - first_played) / (1000 * 60 * 60)

        if play_time_hours < 1:
            return TextFormat.Colorize(f"{play_time_hours * 60:.0f}分钟", TextFormat.BLUE)  # 时间用蓝色
        elif play_time_hours < 24:
            return TextFormat.Colorize(f"{play_time_hours:.1f}小时", TextFormat.BLUE)  # 时间用蓝色
        else:
            return TextFormat.Colorize(f"{play_time_hours / 24:.1f}天", TextFormat.BLUE)  # 时间用蓝色
    ...
    def _ConversionDate(timestamp: int) -> str:
        ...
        try:
            ...
            return TextFormat.Colorize(dt.strftime("%Y-%m-%d %H:%M:%S"), TextFormat.BLUE)  # 时间用蓝色
        except:
            return TextFormat.Colorize("无效时间", TextFormat.RED)  # 错误用红色
    ...
    def _ShowAllInventoryItems(self, inventory: list, ender_items: list):
        ...
        if not inventory:
            ...
            self._ShowPlayerInventoryItems(inventory, damaged_items)  # 传入damaged_items列表
        ...
        if not ender_items:
            ...
            self._ShowEnderChestItems(ender_items, damaged_items)  # 传入damaged_items列表

        # 如果有损坏的物品，在最后显示
        if damaged_items:
            log_info(f"{TextFormat.RED}在解析过程中出现了如下的错误数据，这可能是NBT数据损坏导致的；")
            for i, (location, slot, error) in enumerate(damaged_items, 1):
                log_warn(f"{i}. {location} - 槽位[{slot}]: {error}")

    # 解析玩家物品栏/热键栏/副手栏/末影箱/装备栏物品
    def _ShowPlayerInventoryItems(self, inventory: list, damaged_items: list):
        # 分类物品
        hotbar = []
        main_inventory = []
        armor = []
        ...
        for item in inventory:
            try:
                ...
                if 0 <= slot < 9:  # 快捷栏
                    ...
                    # 未知的槽位
                    hotbar.append((slot, f"{TextFormat.RED}[损坏的物品]x? - 未知槽位{TextFormat.CLEAR}"))

            except Exception as e:
                error_msg = f"解析错误: {str(e)}"
                slot = item.get('Slot', -1)
                damaged_items.append(("物品栏", slot, error_msg))

                # 在对应位置显示损坏标记
                if 0 <= slot < 9:  # 快捷栏
                    hotbar.append((slot + 1, f"{TextFormat.RED}[损坏的物品]x?{TextFormat.CLEAR}"))
                elif 9 <= slot < 36:  # 主物品栏
                    main_inventory.append((slot - 8, f"{TextFormat.RED}[损坏的物品]x?{TextFormat.CLEAR}"))
                elif 100 <= slot < 104:  # 盔甲栏
                    armor_slots = {100: "靴子", 101: "护腿", 102: "胸甲", 103: "头盔"}
                    armor.append((armor_slots.get(slot, "未知"), f"{TextFormat.RED}[损坏的物品]x?{TextFormat.CLEAR}"))
                elif slot == -106:  # 副手
                    offhand.append((f"副手", f"{TextFormat.RED}[损坏的物品]x?{TextFormat.CLEAR}"))
                else:
                    # 未知槽位也记录到热键栏显示
                    hotbar.append((slot, f"{TextFormat.RED}[损坏的物品]x? - 未知槽位{TextFormat.CLEAR}"))
    ...
    # 解析并显示末影箱物品
    def _ShowEnderChestItems(self, ender_items: list, damaged_items: list):
        for i, item in enumerate(ender_items, 1):
            try:
                ...
                error_msg = f"解析错误: {str(e)}"
                damaged_items.append(("末影箱", i, error_msg))
                log_info(f"  [{i}] {TextFormat.RED}[损坏的物品]x?{TextFormat.CLEAR}")
    ...
    def _GetDimensionData(self):
        ...
        if portal_cooldown > 0:
            log_info(f"传送门冷却: {TextFormat.YELLOW}{portal_cooldown / 20:.1f}秒")
        else:
            log_info(f"传送门冷却: {TextFormat.RED}无")
    ...
    def _ParseItem(item: dict) -> str:
        try:
            # 安全地获取字段值，提供默认值
            item_id = str(item.get('id', 'unknown')).replace('minecraft:', '')
            count = int(item.get('Count', 1))  # 默认数量为1
            ...
            # 重新抛出异常，让调用者处理
            raise Exception(f"物品解析失败: {str(e)}")

    # 处理游戏模式名
    @staticmethod
    def _ProcessGamemode(mode_id: int) -> str:
        modes = {
            0: f"{TextFormat.BRIGHT_YELLOW}生存模式",
            1: f"{TextFormat.GREEN}创造模式",
            2: f"{TextFormat.CYAN}冒险模式",
            3: f"{TextFormat.BLUE}旁观模式"
        }
    ...
    @staticmethod
    def _ProcessPotionEffectName(effect_id: int) -> str: # 直接硬编码（感谢zh.minecraft.wiki的馈赠）
        effects = {
    1: "迅捷",
    2: "缓慢",
    3: "急迫",
...
def main():
    parser = argparse.ArgumentParser(
        description='解析玩家NBT数据文件',
        formatter_class=argparse.RawTextHelpFormatter
    )

    # 必需参数
    parser.add_argument('-np', '--nbt-path', required=True,
                       help='指定NBT文件路径')

    parser.add_argument('-about', '--about', action='version',
                        version='BeaconEX NBT解析器\n源仓库：https://github.com/GongSunFangYun/BeaconEX',
                        help='显示关于信息')

    args = parser.parse_args()

    # 参数验证
    if not os.path.exists(args.nbt_path):
            log_error(f"指定的NBT文件不存在: {args.nbt_path}")
            sys.exit(1)
